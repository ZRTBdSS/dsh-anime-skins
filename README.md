...
| 插件 | 状态 | 说明 |
| --- | --- | --- |
| dsh-usage-stats | 兼容 | 侧栏隐藏后显示在底部统计行附近 |
| dsh-better-sidebar | 兼容 | 预留右侧空间，可手动展开 |
| dsh-smooth-stream | 兼容 | 配合 dsh-asuna-bridge 使用 |
| dsh-annotation | 兼容 | 视觉已适配 |
| dsh-composer-expand | 兼容 | 输入区扩展样式已适配 |
| dsh-chat-timeline | 兼容 | 时间线样式已适配 |
